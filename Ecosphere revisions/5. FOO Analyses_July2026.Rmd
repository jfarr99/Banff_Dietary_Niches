...
spp_data <- spp_data %>% 
  ...
  mutate(form= if_else(is.na(form), "other", form)) %>% 
    mutate(form= if_else(form == "graminoid", "grass", form)) %>% 
...
ggplot(spp_data,
  ...
  scale_fill_manual(values = c("gold", "lightyellow2", "darkorchid", "forestgreen", "brown", "lightgrey"),
                    na.value = "grey90") 

ggsave("Outputs/FOO_all_species_diets_noSeason_july2026.jpeg", width = 6, height = 6)
...
physeq_sankey_PLOTdat <- physeq_sankey_data %>% 
  ...
  mutate(form= if_else(is.na(form), "other", form)) %>% 
  mutate(form= if_else(form == "graminoid", "grass", form)) 
table(physeq_sankey_PLOTdat$form)
...
ggplot(physeq_sankey_PLOTdat,
       aes(x =Species, fill = form, y = POO*100)) +
geom_col(color = "grey20", alpha = 0.8) + 
...
ggsave("Outputs/FOO_seasonal_diets_july2026.jpeg", width = 5, height = 6)
...
No point doing diversity analyses - they'll be more or less the same between RRA and FOO! 
...
ordination_RRA_dat$Season = factor(ordination_RRA_dat$Season, levels = c("Winter", "Spring", "Summer"))
...
ordination_RRA <- ggplot(ordination_RRA_dat, aes(x = NMDS1, y = NMDS2*-1, 
                                                 ...
                                                 shape = Species)) +
  geom_point(size = 1.5) + 
  stat_ellipse(aes(fill = Species, linetype = Species), geom = "polygon", alpha = 0.5, size = 0.8, type = "norm", level = 0.38) + 
...
ordination_RRA
ggsave("Outputs/FOO_seasonal_core_species_dietoord_August2026.jpeg", ordination_RRA, height = 7.5, width = 9)
...
sample_data(physeq_foo)$feeding_mode <- if_else(sample_data(physeq_foo)$Species == "Bison" | 
                                ...
                                if_else(sample_data(physeq_foo)$Species== "Deer", "Browser", "Mixed"))

feeding_mode_dat <- subset_samples(physeq_foo, (Species != "Moose" & Species != "Goat"))
...
write.csv(pairwise.species, "Outputs/FOO_pairwise_comparisons_across_all_species_july2026.csv")
...
physeq_foo_subset <- subset_samples(physeq_foo, Species %in% c("Bison", "Elk", "BighornSheep"))
...
ordination_RRA <- ggplot(ordination_RRA_dat, aes(x = NMDS1, y = NMDS2*-1, 
  ...
  geom_point(size = 1.5) + 
  stat_ellipse(aes(fill = Species, linetype = Species), type = "norm", level = 0.38, geom = "polygon", alpha = 0.2, size = 0.8) + 
 ...
 # geom_text(aes(label = ID_Number), vjust = -0.5) + 
  facet_wrap(.~Biszone, nrow = 2,  strip.position = "top") + 
    scale_fill_manual(values = c("#586028","tan1",  "#00A1B7")) + 
  scale_color_manual(values = c("#586028","tan1",  "#00A1B7")) + 
  scale_shape_manual(values = c(16, 15,  17)) + 
  scale_linetype_manual(values = c("solid", "dashed",  "dotted")) + 
