...
size_mat <- df %>%
  pivot_wider(names_from = species2, values_from = body_size_diff) %>%
  column_to_rownames("species1")

# Ensure symmetric (Mantel requires square distance objects)
diss_mat <- as.matrix(diss_mat)
size_mat <- as.matrix(size_mat)
...
bis_bhs <- subset_samples(physeq_rra_core, Zone == "In" & Season == "Winter" & (Species == "Bison" | Species == "BighornSheep"))
diet_matrix <- t(otu_table(bis_bhs)) # get our community matrix
covar_table <- data.frame(sample_data(bis_bhs))
...
bis_bhs <- subset_samples(physeq_rra_core,Zone == "In" & Season == "Spring" & (Species == "Bison" | Species == "BighornSheep"))
diet_matrix <- t(otu_table(bis_bhs)) # get our community matrix
covar_table <- data.frame(sample_data(bis_bhs))
...
bis_elk <- subset_samples(physeq_rra_core, Zone == "In" & Season == "Summer" & (Species == "Bison" | Species == "Elk"))
diet_matrix <- t(otu_table(bis_elk)) # get our community matrix
covar_table <- data.frame(sample_data(bis_elk))
...
bis_bhs <- subset_samples(physeq_rra_core,Zone == "In" &  Season == "Summer" & (Species == "Bison" | Species == "BighornSheep"))
diet_matrix <- t(otu_table(bis_bhs)) # get our community matrix
covar_table <- data.frame(sample_data(bis_bhs))
